...
/* renamed from: com.predictezy.fp.BundesligaStandings */
public class BundesligaStandings extends AppCompatActivity {
    Button backButton;
    Toast calculate;
    Toast connect;
    int count = 0;
    ...
    public void onCreate(Bundle bundle) {
        ...
        setContentView(R.layout.bundesliga_standings);
        ...
        adView.setAdUnitId("ca-app-pub-2924639732611391/2450845541");
        ...
        new Thread() {
            public void run() {
                String str = "null";
                BundesligaStandings BundesligaStandings = BundesligaStandings.this;
                BundesligaStandings.db = BundesligaStandings.getItems();
                while (String.valueOf(BundesligaStandings.db).equals("null") && BundesligaStandings.count<10) {
                    
                    if (String.valueOf(BundesligaStandings.this.db).equals(str) && BundesligaStandings.this.count < 10) {
                        try {
                            sleep(1000);
                            BundesligaStandings.this.count++;
                            System.out.println(BundesligaStandings.this.count);
                        } catch (InterruptedException e) {
                            e.printStackTrace();
                        }
                    }
                }
                if (String.valueOf(BundesligaStandings.this.db).equals(str)) {
                    BundesligaStandings.this.runOnUiThread(new Runnable() {
                        public void run() {
                            BundesligaStandings.this.error.show();
                            BundesligaStandings.this.startActivity(new Intent(BundesligaStandings.this, HomePage.class));
                        }
                    });
                } else {
                    BundesligaStandings.this.runOnUiThread(new Runnable() {
                        public void run() {
                            BundesligaStandings.this.Visibility();
                        }
                    });
                }
            }
    ...
    public ArrayList<HashMap<String, String>> getItems() {
        StringRequest stringRequest = new StringRequest(0, "https://script.google.com/macros/s/AKfycbwLJwoWVc_GvpcH2ZNrHq1ISsFub3ACZxKh0v6w4w5BEwlPv2I/exec?action=getItems", new Listener<String>() {
            public void onResponse(String str) {
                BundesligaStandings BundesligaStandings = BundesligaStandings.this;
                BundesligaStandings.db = BundesligaStandings.parseItems(str);
            }
        }, new ErrorListener() {
    ...
    private void startcalc() {
        ...
        while (true) {
            ...
            dArr2[i3 - 1] = (pow * 34.0d) + ((d3 - (d2 * pow)) / d5);
            i3++;
            dArr = dArr2;
            i2 = 18;
        }
