...
/* renamed from: com.predictezy.fp.LaLigaStandings */
public class LaLigaStandings extends AppCompatActivity {
    Button backButton;
    Toast calculate;
    Toast connect;
    int count = 0;
    ...
    public void onCreate(Bundle bundle) {
        ...
        setContentView((int) R.layout.la_liga_standings);
        ...
        adView.setAdUnitId("ca-app-pub-2924639732611391/5077008884");
        ...
        new Thread() {
            public void run() {
                String str = "null";
                LaLigaStandings LaLigaStandings = LaLigaStandings.this;
                LaLigaStandings.db = LaLigaStandings.getItems();
                while (String.valueOf(LaLigaStandings.db).equals("null") && LaLigaStandings.count<10) {
                    
                    if (String.valueOf(LaLigaStandings.this.db).equals(str) && LaLigaStandings.this.count < 10) {
                        try {
                            sleep(1000);
                            LaLigaStandings.this.count++;
                            System.out.println(LaLigaStandings.this.count);
                        } catch (InterruptedException e) {
                            e.printStackTrace();
                        }
                    }
                }
                if (String.valueOf(LaLigaStandings.this.db).equals(str)) {
                    LaLigaStandings.this.runOnUiThread(new Runnable() {
                        public void run() {
                            LaLigaStandings.this.error.show();
                            LaLigaStandings.this.startActivity(new Intent(LaLigaStandings.this, HomePage.class));
                        }
                    });
                } else {
                    LaLigaStandings.this.runOnUiThread(new Runnable() {
                        public void run() {
                            LaLigaStandings.this.Visibility();
                        }
                    });
                }
            }
    ...
    public ArrayList<HashMap<String, String>> getItems() {
        StringRequest stringRequest = new StringRequest(0, "https://script.google.com/macros/s/AKfycbwC32Sf4zavFFb7xnu0hNbq8HXiJn7_taobBfPFrGHdjPm9WU0/exec?action=getItems", new Listener<String>() {
            public void onResponse(String str) {
                LaLigaStandings LaLigaStandings = LaLigaStandings.this;
                LaLigaStandings.db = LaLigaStandings.parseItems(str);
            }
        }, new ErrorListener() {
